...
export async function createServiceOrderFromCotacao(cotacaoId: string) {
  ...
  if (!cotacao.equipmentId) return { error: 'Cotação sem equipamento vinculado' }

  // Verificar se já tem OS
  const existing = await prisma.serviceOrder.findFirst({
    where: { cotacaoId },
  })
  if (existing) return { error: 'Já existe uma OS para esta cotação' }
