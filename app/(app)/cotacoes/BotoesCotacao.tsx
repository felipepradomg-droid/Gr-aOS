...
export default function BotoesCotacao({
  ...
  function gerarPDF() {
    const conteudo = `
      ...
      <head>
        <meta charset="UTF-8">
        <title>Cotação - ${clienteNome}</title>
        <style>
          ...
          .valor-label { font-size: 13px; color: #888; }
          .valor-num { font-size: 28px; font-weight: 900; color: #F97316; }
          .descricao { background: #f5f5f5; padding: 16px; border-radius: 8px; line-height: 1.6; }
          .footer { margin-top: 40px; padding-top: 20px; border-top: 1px solid #eee; font-size: 12px; color: #888; text-align: center; }
          .badge { display: inline-block; background: #F97316; color: white; padding: 4px 12px; border-radius: 999px; font-size: 12px; font-weight: 700; }
        </style>
      </head>
      <body>
        <div class="header">
          <div class="logo">GrúaOS</div>
          <div class="titulo">Proposta Comercial</div>
        </div>

        <div class="campo">
          <div class="label">Cliente</div>
          <div style="font-size: 20px; font-weight: 700;">${clienteNome}</div>
        </div>
        ...
        <div class="campo">
          ...
          <div class="descricao">${observacoes}</div>
        </div>` : ""}

        ${valor ? `
        <div class="campo" style="margin-top: 24px;">
          <div class="valor-label">Valor total</div>
          <div class="valor-num">${valor.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}</div>
        </div>` : ""}

        <div class="campo" style="margin-top: 20px;">
          <span class="badge">Proposta válida por 7 dias</span>
        </div>

        <div class="footer">
          Gerado por GrúaOS · gruaos.vercel.app · gruaossolucoes@gmail.com
        </div>
      </body>
      </html>
    `;
